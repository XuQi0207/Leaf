...
char *fullpath;
{
	...
	struct Cns_file_transform_stat t_statbuf;
        if (tflag){
            if(Cns_get_Data_daemon(fullpath, &t_statbuf) < 0)
                return (-1);
            return(listtfile(fullpath, &t_statbuf));
        }
...
printf ("%s %3d %-8.8s %-6.6s %s %s ",
    ...
    u64tostr (statbuf->size, tmpbuf, 20), timestr);
}
